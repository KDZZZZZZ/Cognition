...
const MATH_BRIDGE_SYNC_OPTIONS = {
  maxScanBlocks: 32,
  neighborRadius: 2,
  enableDisplay: true,
  enableInline: true,
...
function applyMathBridgeTransforms(instance: Editor | null) {
  if (!instance) return;
  let guard = 0;
  while (guard < MATH_BRIDGE_SYNC_OPTIONS.maxInitialTransforms) {
    const tr = createBridgeTransaction(instance.state, {
      options: MATH_BRIDGE_SYNC_OPTIONS,
    });
    if (!tr) break;
